...
for idx, box in enumerate(areas):
    ...
    # OUT 시간 패턴 확인 (숫자가 있으면 정상, 없으면 미출근)
...
# 미출근자 필터링
missed = df[df['출근상태'] == '미출근']['이름'].tolist()

# 미출근자 있을 경우에만 디스코드 알림
if missed:

    mention_text = "\n".join(
        [f"<@{discord_ids[name]}> 님, ❗**입실**❗ 찍어주세요!" for name in missed if name in discord_ids]
    )
    message = f" **미출근 인원 알림**\n{mention_text}"
